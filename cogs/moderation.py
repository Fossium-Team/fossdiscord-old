...
class Moderation(commands.Cog):
    ...
    @staticmethod
    def timeconvertion(time):  # Time conversion
        ...
        if time[-1].isalpha() is True and time[
            0].isdigit() and lettercount == 1 and letters_inside in conversion and time.isalnum() is True:
            timeconverted = int(to_convert) * conversion[time[-1]]
    ...
    async def purge(self, ctx, amount=None):
        if amount is None:
            ...
            em.add_field(name=f"{config.prefix}purge <number of messages to purge>",
                         value="Purge certain amount of messages.")
            em.add_field(
                name=f"{config.prefix}purge <mention> <how many messages to look back for message sent by user>",
                value="Purge certain amount of messages sent by a certain user.")
        ...
        if os.path.isfile(f"settings/logging-{ctx.guild.id}.json"):
            ...
            em = discord.Embed(title=f"{ctx.author} has purged {amount} messages in {ctx.message.channel}",
    ...
    async def _user(self, ctx, user: discord.Member = None, amount=10):
        ...
        if os.path.isfile(f"settings/logging-{ctx.guild.id}.json"):
            ...
            loggingchannel = loggingjson["data"]["logging"]["channel"]
            channel = self.bot.get_channel(int(loggingchannel))
            em = discord.Embed(title=f"{user} has purged all messages of {user} in the last {amount} messages",
                               color=discord.Color.orange())
    ...
    async def kick(self, ctx, user: discord.Member = None, *, reason=None):
        ...
        if os.path.isfile(f"settings/logging-{ctx.guild.id}.json"):
            ...
            loggingchannel = loggingjson["data"]["logging"]["channel"]
            channel = self.bot.get_channel(int(loggingchannel))
            em = discord.Embed(title=f"{user.display_name} has been kicked", color=discord.Color.orange())
            em.set_author(name=user.display_name, icon_url=user.avatar_url)
            if not reason:
                em.add_field(name="Reason", value="no reason provided")
            else:
    ...
    async def mute(self, ctx, user: discord.Member = None, mutetime="15m"):
        ...
        if discord.utils.get(ctx.guild.roles, name="Muted"):
            ...
            try:
                ...
                em = discord.Embed(
                    title="A role named `Muted` does not exist in your server, please create it first. And also make sure to create overrides for the channels you don't want a muted user speaking in.",
        ...
        if converted is not False:
            ...
            em = discord.Embed(title=f"`{user.display_name}` has been muted for " + "`{}`".format(str(mutetime)),
                               color=discord.Color.orange())
            await ctx.send(embed=em)
        ...
        if os.path.isfile(f"settings/logging-{ctx.guild.id}.json"):
            ...
            loggingchannel = loggingjson["data"]["logging"]["channel"]
            channel = self.bot.get_channel(int(loggingchannel))
            em = discord.Embed(title=f"{user.display_name} has been muted", color=discord.Color.orange())
            em.set_author(name=user.display_name, icon_url=user.avatar_url)
    ...
    async def delwarning(self, ctx, user: discord.Member = None, casenumber=None):
        ...
        if not os.path.isdir("warnings"):
            ...
            try:
                ...
                with open(f"warnings/warnings-{ctx.guild.id}.json", "w") as file:
                    json.dump(data, file, indent=4)
                em = discord.Embed(title=f"Successfully cleared all the warnings of {user.display_name}",
                                   color=discord.Color.green())
                await ctx.send(embed=em)
            except (IndexError, KeyError):
                em = discord.Embed(title="The warning you are trying to remove does not exist",
                                   color=discord.Color.red())
                await ctx.send(embed=em)
                return
        else:
            ...
            try:
                ...
                await ctx.send(embed=em)
                return
            em = discord.Embed(title="Successfully removed that warning", delete_after=10.0,
                               color=discord.Color.green())
            await ctx.send(embed=em)
    ...
    async def modnick(self, ctx, *, user: discord.Member = None):
        if user is None:
            ...
            em.add_field(name=f"{config.prefix}modnick <user ID or mention>",
                         value='Moderates the nickname of a user or a bot (sets the nickname to "ModdedNick-(random letters and numbers)"')
            await ctx.send(embed=em)
        else:
    ...
    async def changenick(self, ctx, user: discord.Member = None, *nick):
        ...
        if user is None:
            em = discord.Embed(title='Usage:', color=discord.Color.blue())
            em.add_field(
                name=f"{config.prefix}changenick <user ID or mention> <new nickname (if not given the nickname will be reset)>",
                value="Changes the nickname of a user or a bot.")
            await ctx.send(embed=em)
